...
class DiscoveryRepository:
    ...
    def get_datasource_connection(self, connection_id: int) -> dict[str, Any] | None:
        query = text("""
            SELECT connection_id, connection_name, database_type, host, port,
                   database_name, username, ssl_enabled, password_plaintext,
                   credential_reference, is_active, discovery_status
            FROM demooc28.datasource_connections
            WHERE connection_id = :connection_id
        """)
        with self.database.connect() as connection:
            row = connection.execute(query, {"connection_id": connection_id}).mappings().one_or_none()
        return dict(row) if row else None

    def get_datasource_for_run(self, run_id: UUID | str) -> dict[str, Any] | None:
        query = text("""
            SELECT datasource.connection_id, datasource.connection_name,
                   datasource.database_type, datasource.host, datasource.port,
                   datasource.database_name, datasource.username,
                   datasource.ssl_enabled, datasource.password_plaintext,
                   datasource.credential_reference, datasource.is_active,
                   datasource.discovery_status
            FROM demooc28.discovery_runs AS run
            JOIN demooc28.datasource_connections AS datasource
              ON datasource.connection_id = run.connection_id
            WHERE run.discovery_run_id = :run_id
        """)
        with self.database.connect() as connection:
            row = connection.execute(query, {"run_id": str(run_id)}).mappings().one_or_none()
        return dict(row) if row else None
    ...
    def create_datasource_connection(
        ...
        """Create a datasource and return only non-password fields."""
        query = text("""
